...
function SignIn() {
    return {
        ...
        signInForm: document.getElementById('sign-in-form') as HTMLFormElement,
        email: document.getElementById('email') as HTMLInputElement,
        password: document.getElementById('password') as HTMLInputElement,
        signInMessage: document.getElementById('signin-message') as HTMLDivElement,
        timeout: null as number | null,

        initSignIn(): void {
            this.signInForm.addEventListener('submit', async (event) => this.verifySavedUser(event), {
        ...
        async verifySavedUser(event: SubmitEvent): Promise<void> {
            ...
            try {
                ...
                setTimeout(() => window.location.href = '/html/seller.html', 500);
            } catch (error: any) {
                this.signInMessage.classList.remove('hidden');
                this.signInMessage.classList.add('block');
