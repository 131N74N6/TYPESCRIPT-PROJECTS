...
class DisplayManager extends DataManager<Data> {
    ...
    private setupRealtimeListener(): void {
        this.loadFromStorage((data) => this.realTimeRendering(data));
    }

    private realTimeRendering(data: Data[]): void {
    ...
    private async selectedItem(id: string): Promise<void> {
        this.dynamicFields.innerHTML = '';
        this.inputSection.style.display = "flex";
        const data = await this.loadFromStorage();
        const getAllData = data.find(dt => dt.id === id); 
