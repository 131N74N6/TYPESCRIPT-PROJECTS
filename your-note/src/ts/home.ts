...
class HomePage extends SupabaseTable<Note> {
    ...
    showAllNotes(notes: Note[]): void {
        ...
        try {
            ...
            noteList.innerHTML = `<div class="text-[1.1rem] text-[#B91372]">${error.message || error}</div>`;
        }
    }
    ...
    createNoteComponent(note: Note): HTMLDivElement {
        ...
        viewButton.onclick = () => window.location.replace(`/src/pages/detail-note.html?id=${note.id}`);
    ...
    async deleteAllNotes(): Promise<void> {
        try {
            ...
            if (this.currentData.size > 0) {
                ...
                noteList.innerHTML = `<div class="text-[1.1rem] text-[#B91372]">No Note Added Recently...</div>`;
            } else {
                throw 'No Note Added Recently...';
            }
        } catch (error: any) {
